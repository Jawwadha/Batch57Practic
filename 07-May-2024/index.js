...
for (let i = 0; i < strings1.length; i++) {
    if (strings1.filter((strings1) => strings1.toLocaleLowerCase().startsWith("c")))
        str.push(strings1[i]);
}
for (let i = 0; i < strings1.length; i++) {
    if (strings1.filter((strings1) => strings1.toLocaleLowerCase().startsWith("c"))) {
...
// Example usage:
const wordArray = ["Apple", "Banana", "Apricot", "Orange", "Avocado"];
const wordsWithA = wordsStartingWithA(wordArray);
